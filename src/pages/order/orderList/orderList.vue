        ...
        <li class="order-item" v-for="item in orderList" :key="item.orderId">
          <router-link :to="'/shop/' + item.shopId">
            <div class="shop-info">
              ...
              <span class="title">{{ item.shopName }} <ion-icon :icon="chevronForwardOutline"></ion-icon></span>
              <span class="status">{{ item.status === 0 ? "待支付" : item.status === 1 ? "配送中" : item.status === 2 ? "已送达" : "已取消" }}</span>
            </div>
          </router-link>
          <div class="shop-active" v-if="item.activities && item.activities[0]">{{ item.activities[0].description }}</div>
          <div class="food-info" @click="goOrderDetail(item.orderId)">
            <div class="food-list">
              <div class="food-item" v-for="food in item.foods" :key="food.id">
          ...
          <div class="buttons">
            ...
            <button class="theme" @click="setOrder(item)">{{ item.status === 0 ? "去支付" : item.status === 1 ? "确认收货" : "再来一单" }}</button>
          </div>
        </li>
      </ul>
