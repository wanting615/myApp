<template>
  <ion-page>
    ...
    <ion-content>
      <div class="user-info">
        <div class="tips">
          <ion-icon :icon="volumeMediumOutline" class="vertical"></ion-icon>
          <span class="vertical">温馨提示: 请适量点餐、避免浪费</span>
        </div>
        <ul class="user-distribution">
          <li class="address" @click="goAddress">
            <span
              class="flex"
            >{{ data.userAddress ? data.userAddress.addressName + data.userAddress.addressDetail : "请选择收货地址" }}</span>
            <ion-icon :icon="chevronForwardOutline"></ion-icon>
          </li>
          <li
            class="username"
            v-if="data.userAddress"
          >{{ data.userAddress.name }} {{ data.userAddress.phone }}</li>
          <li class="clear">
            <span class="flex">立即送出</span>
            <span class="theme-color" v-if="data.orderTime">大约{{ data.orderTime }}送到</span>
            <ion-icon :icon="chevronForwardOutline"></ion-icon>
          </li>
          <li class="clear">
            <span class="flex">支付方式</span>
            <span class="theme-color">支付宝支付</span>
            <ion-icon :icon="chevronForwardOutline"></ion-icon>
          </li>
        </ul>
      </div>
      ...
      <div class="foods">
        <ul>
          ...
          <li>
            <span class="flex">
              配送费({{ data.deliveryInfo.type }})
              <i class="tip">?</i>
            </span>
            <span class="bold">
              <i>¥</i>
              {{ data.deliveryInfo.price }}
            </span>
          </li>
          <li class="reduction">
            <span class="flex">免配送费</span>
            <span class="bold">
              <i>¥</i>3.0
            </span>
          </li>
          <li v-if="data.activities">
            <span class="icon-font">减</span>
            <span class="flex">店铺{{ data.activities.description }}</span>
            <span class="bold redColor">-¥{{ data.activities.reduceMoney }}</span>
          </li>

          <li @click="goHongbao">
            <span class="icon-font">¥</span>
            <span class="flex">红包抵用卷</span>
          ...
          <li class="warn-tag">
            <span class="icon-font warn">!</span> 已为您选择满减,比折扣节省2
          </li>
          <li class="total-money">
            ...
            <span>
              已优惠
              <span class="redColor">¥{{ getYouhui }}</span>
            </span>
            <span class="total">
              小计¥
              <span>{{ realMoney }}</span>
      ...
      <div class="orther-info">
        <ul>
          ...
          <li @click="selecTableware">
            ...
            <span class="gray-color">{{ data.tableWare ? data.tableWare : "必选" }}</span>
            <ion-icon :icon="chevronForwardOutline"></ion-icon>
          </li>
          <li>
            <span class="flex bold">发票</span>
            <span class="gray-color" v-if="!data.isInvoice">该店暂不支持线上发票,请电话联系商户</span>
