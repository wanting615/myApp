<template>
  <div class="list">
    <div class="item" v-for="item in data.recommendList" :key="item.id">
      <div class="shop-name">
        ...
        <div class="fr">
          <ion-icon src="assets/svg/follow.svg"></ion-icon>
          <span class="vertical">关注</span>
        </div>
      </div>
      ...
      <div class="shop-info">
        <div class="shop-img">
          <img :src="config.imagePath + item.shopInfo.image_path" />
        </div>
        <div class="shop-item">
          <div class="item-name bolder">
            {{ item.shopInfo.name }}
            <span>{{ item.shopInfo.order_lead_time }}分钟送达</span>
          </div>
          <div class="active">
            <span class="shop-scroe">{{ item.shopInfo.rating }}分</span>
            <span
              v-if="item.shopInfo.activities && item.shopInfo.activities.length > 0"
            >{{ item.shopInfo.activities[0].description }}</span>
      ...
      <div class="comments">
        ...
        <div @click="repay(item)">
          <ion-icon :src="chatbubbleEllipsesOutline"></ion-icon>
          <span>{{ item.replys }}</span>
        </div>
        <div>
        ...
        <div>
          <ion-icon :src="ellipsisVerticalOutline"></ion-icon>
        </div>
      </div>
    </div>
  </div>
